...
local function dap_configure()
    ...
    u.keymap_buf("n", "<Leader>dr", function()
        dap.repl.toggle()
    end, "Open debug REPL")
    u.keymap_buf("n", "<Leader>db", function()
        dap.toggle_breakpoint()
    end, "Toggle breakpoint")
    u.keymap_buf("n", "<Leader>dX", function()
        dap.clear_breakpoints()
    end, "Clear all breakpoint")
    u.keymap_buf("n", "<Leader>dp", function()
        dap.continue()
    end, "Continue execution")
    u.keymap_buf("n", "<Leader>du", function()
        dap.run_to_cursor()
    end, "Run to cursor")
    u.keymap_buf("n", "<Leader>dh", function()
        dap.step_out()
    end, "Step out")
    u.keymap_buf("n", "<Leader>dj", function()
        dap.step_over()
    end, "Step over")
    u.keymap_buf("n", "<Leader>dk", function()
        dap.restart_frame()
    end, "Restart frame")
    u.keymap_buf("n", "<Leader>dl", function()
        dap.step_into()
    end, "Step into")

    u.keymap_buf("n", "<Leader>td", function()
        dapui.toggle()
